...
exports.scheduledBackups = exports.checkRecurringProfiles = void 0;
...
// --- Helpers ---
function calculateNextRunDate(daysOfWeek, time) {
    const [hours, minutes] = time.split(':').map(Number);
    const now = new Date();
    const todayIndex = (0, date_fns_1.getDay)(now);
    ...
    if (nextDayIndex > todayIndex) {
        ...
        nextDate = (0, date_fns_1.addDays)(now, nextDayIndex - todayIndex);
    }
    else {
        // Next day is in the next week
        nextDate = (0, date_fns_1.addDays)(now, 7 - (todayIndex - nextDayIndex));
    }
    // Set time
...
async function copyCollection(firestore, sourcePath, destPath) {
    const sourceRef = firestore.collection(sourcePath);
    // Use smaller chunks to avoid memory issues with large collections
    const limit = 500;
    let lastDocSnapshot = null;
    while (true) {
        let query = sourceRef.orderBy(admin.firestore.FieldPath.documentId()).limit(limit);
        if (lastDocSnapshot) {
        ...
        if (documents.size < limit)
            break;
        lastDocSnapshot = documents.docs[documents.docs.length - 1];
    }
}
async function deleteCollectionByPath(firestore, collectionPath, batchSize) {
...
//     }
// }
async function cleanupOldBackups(accountId, retentionDays) {
    const now = new Date();
...
// --- Cloud Functions ---
exports.checkRecurringProfiles = (0, scheduler_1.onSchedule)("every day 06:00", async (event) => {
    const now = new Date();
    ...
    for (const doc of snapshot.docs) {
        try {
            ...
            const newOsData = Object.assign(Object.assign({}, profile.templateData), { recurrenceProfileId: profile.id, createdAt: admin.firestore.FieldValue.serverTimestamp() });
...
exports.scheduledBackups = (0, scheduler_1.onSchedule)("every day 01:00", async (event) => {
    const now = new Date();
    ...
    for (const doc of accountsSnap.docs) {
        ...
        if (!lastBackupDate) {
            ...
            const daysSinceLastBackup = (0, date_fns_1.differenceInDays)(now, (0, date_fns_1.parseISO)(lastBackupDate));
