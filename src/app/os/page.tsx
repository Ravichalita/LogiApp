...
export default function OSPage() {
    ...
    const handleQuickAction = async (action: 'edit' | 'finalize' | 'pdf' | 'delete' | 'swap' | 'cancelRecurrence', item: PopulatedRental | PopulatedOperation) => {
        if (!accountId) return;
            ...
            case 'swap':
                if (isRental) {
                    ...
                    const query = new URLSearchParams({
                        swapOriginId: rental.id, // Pass the old rental ID to be finalized later
            ...
            case 'cancelRecurrence': {
                if (item.recurrenceProfileId) {
                    ...
                    if (result.message === 'success') {
                        ...
                        const updateState = (prevItems: any[]) => prevItems.map(i =>
                            i.id === item.id
                                ? { ...i, recurrenceProfileId: null }
                                : i
                        );
                        if (isRental) setRentals(updateState);
                        else setOperations(updateState);
                    } else {
                                ...
                                <DraggableActionCard
                                    ...
                                    actions={
                                        <>
                                            ...
                                            {canEditRentals && rental.recurrenceProfileId && <Button variant="outline" size="lg" className="h-20 flex-col gap-2" onClick={() => handleQuickAction('cancelRecurrence', rental)}><Repeat className="h-6 w-6 text-destructive" /> Cancelar Recorrência</Button>}
                                        ...
                                        <AccordionItem value={rental.id} className="border-none">
                                            <Card className="relative h-full flex flex-col border rounded-lg shadow-sm overflow-hidden bg-card">
                                                ...
                                                {rental.recurrenceProfileId && (
                                                    <div className="absolute top-2 right-3 flex items-center gap-1.5 text-xs font-bold text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full">
                                ...
                                <DraggableActionCard
                                    ...
                                    actions={
                                        <>
                                            ...
                                            {canEditOp && op.recurrenceProfileId && <Button variant="outline" size="lg" className="h-20 flex-col gap-2" onClick={() => handleQuickAction('cancelRecurrence', op)}><Repeat className="h-6 w-6 text-destructive" /> Cancelar Recorrência</Button>}
