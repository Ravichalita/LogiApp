...
import { CheckCircle, MapPin, Edit, Trash2, TriangleAlert, CircleDollarSign, CalendarDays, MoreVertical, XCircle, FileText, Hash, Truck, Milestone, Route, Clock } from 'lucide-react';
...
export function RentalDetailsDialog({ rental, children }: RentalDetailsDialogProps) {
    ...
    <Dialog>
        ...
        <DialogContent className="max-h-[90vh] flex flex-col p-0 gap-0">
            <DialogHeader className="p-6 pb-4">
                <DialogTitle className="font-headline text-2xl">Detalhes da OS #{rental.sequentialId}</DialogTitle>
                <DialogDescription>
                    {rental.client?.name}
                </DialogDescription>
            </DialogHeader>
            <div className="flex-grow overflow-y-auto px-6 space-y-4">
                ...
                {!isOperation && rental.dumpster && (
                     <div className="flex items-start gap-3">
                        <CacambaIcon className="h-5 w-5 text-muted-foreground mt-1 shrink-0" />
                        <div className="flex flex-col">
            ...
            <div className="flex w-full items-center gap-2 mt-auto p-6 bg-muted/50 border-t">
