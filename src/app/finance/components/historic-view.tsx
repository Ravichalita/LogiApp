...
export default function HistoricView({ items: allHistoricItems, team, account, permissions, isSuperAdmin }: HistoricViewProps) {
    ...
    useEffect(() => {
        setLocalItems(allHistoricItems);
    }, [allHistoricItems]);
    ...
    }, [localItems, activeTab, selectedYear, selectedMonth, selectedResponsible]);
    ...
    const handleEditSuccess = () => {
        setIsEditing(false);
        setItemToEdit(null);
        // We rely on revalidation or page refresh here as getting the full updated item is complex
        // Ideally we would update local state here too
    ...
    const yearlyRevenue = localItems
    ...
    const previousMonthRevenue = useMemo(() => {
        ...
        return localItems
            ...
            .reduce((acc, item) => acc + (item.totalValue || 0), 0);
    }, [localItems, selectedMonth, selectedYear]);
